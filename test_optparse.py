...
for iDate in a[0:len(a)]:
    df_sub = df.loc[df.index == iDate]
    df_sub.loc[:,'RANK'] = df_sub.ix[:, 'Quote'].rank()
    df_sub.loc[:,'B_Q'] = pd.qcut(df_sub['RANK'], 4, labels=["Q1", "Q2", "Q3", "Q4"])
...
df[['B_MKT', 'B_Q1', 'B_Q2', 'B_Q3', 'B_Q4', 'B_AUTOS', 'B_CDRTL', 'B_CNSDA', 'B_CNSSV','B_MEDIA','B_SIZE']]