...
from macpy.utils.Utilities import parseISODate

matplotlib.use('Agg')
MAP_MATURITIES = {0.25:'3M', 0.5:'6M', 1:'1Y', 2:'2Y', 5:'5Y', 10:'10Y', 30: '30Y'}


def makeTempFile(curveshortnames):
    import tempfile
    fd, tmpfile = tempfile.mkstemp(prefix='tmp', dir='.')
    os.write(fd,'CurveShortName\n%s' % '\n'.join(curveshortnames.split(';')))
    os.close(fd)
    return tmpfile 
...
class UsageHelp:
    ...
    def commandLineExamples():
        ...
        python CurveControl.py -f usgraph.jpg -d 2013-09-20 -c us.usd.gvt.zc -s prod-mac-mkt-db
        """
...
class CurvePlot:
    r"""
    Base class containing core ploting logic.
    """
    __ypos=0.0
    __xpos=.5
    ...
    def plot_3DHist(self, df, ax, server, alpha=0.8, textRot=80):
        ...
        [lenDates, lenTerms] = curveHist.shape
        ...
        ax.plot_surface(X, Y, Z, rstride=1, cstride=2, alpha=alpha, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    ...
    def plot_history(self, dateRange, sumDfList, server='Saturn', dim='3d'):
        ...
        if (dim == '2dcomp'):
            ...
            if (len(self.df)>0):
                ...
                self.df.TradeDate = self.df.TradeDate.apply(lambda x: parseISODate(x))
                ###
            ...
            if (len(self.dfFinal)>0):            
                ...
                #self.dfFinal.TradeDate = self.dfFinal.TradeDate.apply(lambda x: x.date())
                ...
                self.dfTestRes = self.dfTestRes.append(pd.DataFrame({'Score':[self.dffOutParSel['score'].sum()],
                                               'Count':[self.dffOutParSel['count'].sum()]}, index=['Baseline']))
            ...
            title = self.name + ' Proposed (Table: CurveNodeQuote)'
            ...
            txtpc = axes[0].text(-.07, 1.08, 'Test\nResults', fontproperties=fontBold,
                                     va = 'top', ha = 'left', transform=axes[0].transAxes, fontsize=5)
            #print self.dff
            plt.tight_layout()
...
class DerivedCurve(CurvePlot):
    ...
    def __init__(self, curveShortName, name=None, dirName='.', finalTable=False):
        ...
        select CurveName as CurveShortName, TimeInYears as InYears, Level as Quote, TradeDate
        from MarketData.dbo.ResearchCurves
        where CurveName in ({2}) and tradedate >= @StartDate and tradedate <= @EndDate
        and Category ='AxiomaBootStrapper'
        order by TradeDate, TimeInYears asc"""
        self.sqlTemplateProd = r"""
        Declare @StartDate datetime = '{0}', @EndDate datetime = '{1}'
        ...
        join MarketData.dbo.{3} cq on cn.curvenodeid = cq.curvenodeid
    ...
    def create_query(self, dateRange=None):
        if (self.name == 'Swap'):
            ...
            curveNames = self.curveShortName.get_curve_names()
            if self.finalTable:
                self.sqlQuery = self.sqlTemplateProd.format(self.dateRange.startDate, self.dateRange.endDate, curveNames, "curvenodequotefinal")
            else:
                self.sqlQuery = self.sqlTemplate.format(self.dateRange.startDate, self.dateRange.endDate, curveNames, "CurveNodeQuote")
        return self.sqlQuery
...
class DBServer:
    ...
    def __init__(self, server, dbName='MarketData', auth='Trusted_Connection=yes', user='MarketDataLoader', password='mdl1234'):
        ...
        self.connstr=r'DRIVER={SQL Server Native Client 10.0};SERVER=%s;DATABASE=%s;%s' % (server, dbName, auth)
...
class CurveShortName:
    ...
    def quick_comp(cls, curveShortName, dateRange, serverBase, serverDev, plotSource=True, maxYears=50):
        ...
        dc = DerivedCurve(curveShortName,name=serverBase+':'+curveShortName, finalTable=self.finalTable)
...
def generateHist(shortName, dateRange, server='Saturn', finalTable=False):
    ...
    df_results = formateRawDataFrame(df)
    return df_results
...
def addlevelbounds(dataframe, column=7, bound=10, window=60):
    df = pd.DataFrame(dataframe.ix[:, column], index=dataframe.index)
    df.columns = ['level']
    df['returns'] = df.diff()
    df['median'] = pd.rolling_median(df['returns'], window=window, min_periods=1)
...
def format_yield_df(df, ttm_set, measures):
    tdf = (df[ttm_set].diff() * np.sqrt(252) * 10000.0).describe()
    tdf = tdf[tdf.index.isin(measures)]
    tdf = tdf.rename(columns=MAP_MATURITIES)
    tdf = tdf[[MAP_MATURITIES[t] for t in ttm_set]]
    tdf = np.round(tdf, 2)
    return tdf


def G9hist(dateRange, server='prod-mac-mkt-db', reportFile='G9History.pdf', inputFile='test.csv'):
    ...
    finalNames = []
    descNames = []
    measures = ['count', ' mean', 'std', 'min', 'max']
    ttm_set = [0.25, 0.5, 1, 2, 5, 10, 30]
    dfs = []
    dfs_final = []
    names = []
    for idx, short_name in G9df.iterrows():
        if (short_name[0] is 'CurveShortName') or (short_name[0].find("#") == 0):
            continue
        if 'GVT.ZC' not in short_name.values[0]:
            continue
        df = generateHist(short_name.values[0], dateRange, server='prod_mac_mkt_db', finalTable=True)
        tdf = format_yield_df(df, ttm_set, measures)
        dfs.append(tdf)
        df_final = generateHist(short_name.values[0], dateRange, server='dev_mac_db', finalTable=False)
        tdf = format_yield_df(df_final, ttm_set, measures)
        dfs_final.append(tdf)
        names.append(short_name.values[0])
        if idx == G9df.shape[0]-1:
            dfa = pd.concat(dfs, axis=0, keys=names)
            dfb = pd.concat(dfs_final, axis=0, keys=names)
    ...
    for idx, shortName in G9df.iterrows():
        if (shortName[0] is 'CurveShortName') or (shortName[0].find("#") == 0):
            continue
        if 'GVT.ZC' not in shortName.values[0]:
            continue
    ...
    for idx, shortName in G9df.iterrows():
        if (shortName[0] is 'CurveShortName') or (shortName[0].find("#") == 0):
            continue
        try:
            fig, ax, dc = CurveShortName.hist_comp(shortName[0], dateRange, sumDfList, server)
    ...
    if (len(sumDfList) > 0):
        ...
        styleN = styles['Code']
        tsText = 'Server: ' + server + ' Date range: ' + dateRange + ' TimeStamp: ' +\
...
def main():
    print "Running CurveControl.py"
    parser = OptionParser()
    parser.add_option("-f", "--outputFile",
                      dest="output_file_name",
                      help="write graph to file. Example: -f US.USD.GVT.ZC.png")
    parser.add_option("-d",
                      "--date",
                      dest="endDate",
                      help="As of date. Example: -d 2013-09-20",
                      metavar="DATE")
    parser.add_option("-c",
                      "--curveShortName",
                      dest="curve_short_name",
                      help="Example: -c US.USD.GVT.ZC")
    parser.add_option("-s", "--server",
                      dest="server",
                      help="Example: -s prod-mac-mkt-db")
    parser.add_option("-r", "--dateRangeFocus",
                      dest="date_range_focus",
                      help="Example: -r 2001-01-01:2014-05-01",
                      default='')

    (options, args) = parser.parse_args()

    input_file_name = makeTempFile(options.curve_short_name)

    G9hist(options.date_range_focus,
           server=options.server,
           reportFile=options.output_file_name,
           inputFile=input_file_name)

    os.remove(input_file_name)
    #output_url = options.filename.replace('/axioma/','http://')
    #print '<a href="%s">Graphs for %s </a>' % (output_url, options.filename)
