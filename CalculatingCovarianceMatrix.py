...
def compute_FactorReturns(d0, d1, RegressionData):
    ...
    B.drop (['B_CNSSV'], axis=1, inplace=True)
    assets = r.index.intersection (B.index)
    resid = r[assets] - B.loc[assets].dot (fr_W)
    Q_prime = B.dot(fcov_W).dot(B.T)


    return fr_W, t_W, fcov_W, rsquared_W
...
for d0,d1 in itertools.izip(date[:-1], date[1:]):
    # print 'date', d1
    (fr_W, t_W, fcov_W, rsquared_W) = compute_FactorReturns(d0, d1, RegressionData)
    fr.append (fr_W)
    tstats.append (t_W)
    fcov.append(fcov_W)
